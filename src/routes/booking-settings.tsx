...
import { Plus, Trash2, Clock, Coffee, Timer, CalendarClock } from "lucide-react";
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <section className="glass-card rounded-2xl p-5 lg:col-span-1 space-y-5 h-fit">
          ...
          <button
            ...
            className="w-full h-10 rounded-lg border border-border text-xs text-muted-foreground hover:text-foreground"
          >
            ...
            {WEEKDAYS.map((d) => {
              ...
              return (
                <div key={d} className={cn(
                  "rounded-xl border p-3 flex flex-wrap items-center gap-3 transition",
                  s.open ? "border-border bg-muted/20" : "border-border/60 bg-muted/10 opacity-70",
                )}>
                  <label className="flex items-center gap-2 min-w-[120px]">
                    ...
                    />
                    <span className="font-semibold text-sm">{dayLabel(d)}</span>
                  </label>
                  <div className="flex items-center gap-2 flex-1 flex-wrap">
                    ...
                    <input
                      ...
                      className="h-9 rounded-lg bg-muted/40 border border-border px-2 text-sm disabled:opacity-50"
                    />
                    ...
                    <input
                      ...
                      className="h-9 rounded-lg bg-muted/40 border border-border px-2 text-sm disabled:opacity-50"
                    />
                    {!s.open && <span className="text-xs text-muted-foreground">مغلق</span>}
                  </div>
                </div>
              );
            })}
